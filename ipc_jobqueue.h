 ...
 *      ipc_jobqueue_capacity(ipc_jobqueue_t* ijq);
 ...
 *      ipc_jobqueue_peekhead(ipc_jobqueue_t* ijq, job_t* dst);
 *      ipc_jobqueue_peektail(ipc_jobqueue_t* ijq, job_t* dst);
 *      ipc_jobqueue_delete(ipc_jobqueue_t* ijq, job_t* dst);
 *
 * These are direct counterparts to jobqueue functions (see jobqueue.h). 
 * This is because, except for ipc_jobqueue_new and ipc_jobqueue_delete, 
 * each ipc_jobqueue function is a wrapper for the corresponding jobqueue
 * function. That is, ipc_jobqueue_capacity calls jobqueue_capacity,
 * ipc_jobqueue_dequeue calls jobqueue_dequeue etc
 *
 * The main difference between a jobqueue and an ipc_jobqueue is that a 
 * jobqueue is allocated on the heap in the address space of a single process,
 * whereas an ipc_jobqueue is a wrapper for a jobqueue that is allocated in 
 * an area of shared memory that is set up as part of ipc object creation and
 * can be mapped to address spaces of multiple processes. That is, a jobqueue 
 * is private to a single process, whereas a jobqueue wrapped by an 
 * ipc_jobqueue can be accessed by multiple processes. If one process 
 * changes the single shared queue then other processes that share it will see
 * the change.
 *
 * Another difference between an ipc_jobqueue and a jobqueue is that, for 
 * simulation purposes, in each ipc_jobqueue function (except new, capacity 
 * and delete), a critical work delay is injected. This is done by calling 
 * do_critical_work with the proc field of the ipc_jobqueue object.
 *
 * At application level, usage of a jobqueue and an ipc_jobqueue are almost
 * identical. The ipc_jobqueue abstracts away from shared memory setup etc.
 * It is trivial to modify an application using a jobqueue to use an 
 * ipc_jobqueue.
 *
 * IMPORTANT:
 ...
 * field is a jobqueue in shared memory.
 * 
 * Parameters:
 ...
 * ipc_jobqueue_capacity(ipc_jobqueue_t* ijq)
 *
 * This is a wrapper for jobqueue_capacity.
 *
 * See the specification of jobqueue_capacity in jobqueue.h.
 *
 * In particular, if the ijq parameter is NULL this function has the 
 * same behaviour as the corresponding jobqueue function.
 */
 ...
 * ipc_jobqueue_dequeue(ipc_jobqueue_t* ijq, job_t* dst)
 *
 * This is a wrapper for jobqueue_dequeue.
 *
 * See the specification of jobqueue_dequeue in jobqueue.h.
 *
 * In particular, if the ijq parameter is NULL this function has the 
 * same behaviour as the corresponding jobqueue function and no critical work 
 * is simulated.
 */
 ...
 *
 * This is a wrapper for jobqueue_enqueue.
 *
 * See the specification of jobqueue_enqueue in jobqueue.h.
 *
 * In particular, if the ijq parameter is NULL this function has the 
 * same behaviour as the corresponding jobqueue function and no critical work 
 * is simulated.
 ...
 * ipc_jobqueue_is_empty(ipc_jobqueue_t* ijq)
 *
 * This is a wrapper for jobqueue_is_empty.
 *
 * See the specification of jobqueue_is_empty in jobqueue.h.
 *
 * In particular, if the ijq parameter is NULL this function has the 
 * same behaviour as the corresponding jobqueue function and no critical work 
 * is simulated.
 ...
 * ipc_jobqueue_is_full(ipc_jobqueue_t* ijq)
 *
 * This is a wrapper for jobqueue_is_full.
 *
 * See the specification of jobqueue_is_full in jobqueue.h.
 *
 * In particular, if the ijq parameter is NULL this function has the 
 * same behaviour as the corresponding jobqueue function and no critical work 
 * is simulated.
 ...
 * ipc_jobqueue_peekhead(ipc_jobqueue_t* ijq, job_t* dst)
 *
 * This is a wrapper for jobqueue_peekhead.
 *
 * See the specification of jobqueue_peekhead in jobqueue.h.
 *
 * In particular, if the ijq parameter is NULL this function has the 
 * same behaviour as the corresponding jobqueue function and no critical work 
 * is simulated.
 ...
 * ipc_jobqueue_peektail(ipc_jobqueue_t* ijq, job_t* dst)
 *
 * This is a wrapper for jobqueue_peektail.
 *
 * See the specification of jobqueue_peektail in jobqueue.h.
 *
 * In particular, if the ijq parameter is NULL this function has the 
 * same behaviour as the corresponding jobqueue function and no critical work 
 * is simulated.
